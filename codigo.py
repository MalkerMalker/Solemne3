...
if opcion == 'Información':
    ...
    if not fila.empty:  # Verifica si existe alguna coincidencia
        ...
        ax1.plot(ejex1, ejey1, label="Línea")
        ax1.scatter(highlightx1, highlighty1, color='red', label="Destacado")
        ax1.axhline(highlighty1, color='green', linestyle='--', label="Referencia")
        ax1.set_title(title)
        ax1.legend()
        st.pyplot(fig1)

        fig2, ax2 = plt.subplots()
        ax2.plot(ejex2, ejey2, label="Línea")
        ax2.scatter(highlightx2, highlighty2, color='red', label="Destacado")
        ax2.axhline(highlighty2, color='green', linestyle='--', label="Referencia")
        ax2.set_title(title2)
        ax2.legend()
        st.pyplot(fig2)
        
        fig3, ax3 = plt.subplots()
        ax3.plot(ejex3, ejey3, label="Línea")
        ax3.scatter(highlightx3, highlighty3, color='red', label="Destacado")
        ax3.axhline(highlighty3, color='green', linestyle='--', label="Referencia")
